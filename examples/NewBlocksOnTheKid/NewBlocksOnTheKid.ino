...
void loop()
{
    ...
    if (irrecv.decode(&results)) {
        ...
        if ( results.value == 0xFF906F ) moveRight += 1;//右
        if ( results.value == 0xFFE01F )  moveLeft += 1;//左
        if ( results.value == 0xFF9867 )  downButton += 1;//下降
        if ( results.value == 0xFFA857 ) aButton += 1;//中间播放
